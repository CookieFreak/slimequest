...
namespace CreativeSpore.RpgMapEditor
{
	...
	public class AutoTileMap : MonoBehaviour 
	{
		...
		{
			public int Type = -1;	// tile idx dependent of tile type ( the animated tileset has some tiles groupped as one and the idx is different )
		...
		public int TileAnimFrame{ get{ return (int)m_tileAnimFrame; } }
		...
		private float m_tileAnimFrame = 0f;
		...
		void Update () 
		{
			...
			m_tileAnimFrame += Time.deltaTime * AnimatedTileSpeed;
			while( m_tileAnimFrame >= 3f ) m_tileAnimFrame -= 3f;
			TileAnimFrameHasChanged = (int)m_tileAnimFrame != __prevTileAnimFrame ;
			__prevTileAnimFrame = (int)m_tileAnimFrame;	
		...
		public void AddAutoTile( int autoTile_x, int autoTile_y, int tile_type, int iLayer )
		{
		...
		public eTileCollisionType GetAutotileCollisionAtPosition( Vector3 vPos )
		{
			...
			return eTileCollisionType.NONE;
		}
		...
		// NOTE: depending of the collType and tilePartType, this method returns the collType or eTileCollisionType.NONE
