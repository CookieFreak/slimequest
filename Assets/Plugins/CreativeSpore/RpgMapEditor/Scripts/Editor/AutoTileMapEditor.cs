					...
					if( isOk )
					{
						...
						for( int i = 0; i < MyAutoTileMap.MapTileWidth; i++ )
						{
							for( int j = 0; j < MyAutoTileMap.MapTileHeight; j++ )
							{
								...
								if( noise < 0.3 ) //water
								{
									MyAutoTileMap.AddAutoTile( i, j, 0, (int)AutoTileMap.eTileLayer.GROUND);
								}
								else if( noise < 0.4 ) // water plants
								{
									MyAutoTileMap.AddAutoTile( i, j, 0, (int)AutoTileMap.eTileLayer.GROUND);
									if( fRand < noise/3 )
										MyAutoTileMap.AddAutoTile( i, j, 5, (int)AutoTileMap.eTileLayer.GROUND_OVERLAY);
								}
								else if( noise < 0.5 && fRand < (1-noise/2) ) // dark grass
								{
									MyAutoTileMap.AddAutoTile( i, j, 32, (int)AutoTileMap.eTileLayer.GROUND);
								}
								else if( noise < 0.6 && fRand < (1-1.2*noise) ) // flowers
								{
									//MyAutoTileMap.AddAutoTile( i, j, 24, (int)AutoTileMap.eTileLayer.GROUND);
									MyAutoTileMap.AddAutoTile( i, j, 144, (int)AutoTileMap.eTileLayer.GROUND);
									MyAutoTileMap.AddAutoTile( i, j, 288+Random.Range(0, 5), (int)AutoTileMap.eTileLayer.GROUND_OVERLAY);
								}
								...
								{
									MyAutoTileMap.AddAutoTile( i, j, 33, (int)AutoTileMap.eTileLayer.GROUND);
								}
							}
						}
						MyAutoTileMap.SaveMap();
		...
		void DrawCollisions()
		{
			...
			for( float y = vTopLeft.y; y <= vBottomRight.y; y+=AutoTileset.TileWorldHeight/4 )
			{
				for( float x = vTopLeft.x; x <= vBottomRight.x; x+=AutoTileset.TileWorldWidth/4 )
				{
					...
					if( collType != AutoTileMap.eTileCollisionType.NONE )
					{
